...
def dfs(n, robots, resources, mins_remaining):
    ...
    if mins_remaining == 1:  # This case => 45x speedup!
        if global_best < resources[3] + robots[3]:
            global_best = resources[3] + robots[3]
        return resources[3] + robots[3]
    if mins_remaining <= 0:
        if global_best < resources[3]:
    ...
    for b, bot in enumerate(bots):
        if (resources[b]//mins_remaining) + robots[b] >= maxes[b]:
            continue
        wait_mins = max([(ceil((r-resources[i])/robots[i]) if robots[i] > 0 else math.inf) if r > 0 else 0 for i,r in enumerate(bot)]) + 1
        if wait_mins != math.inf:
            if wait_mins >= mins_remaining:
                continue
            new_robots = tuple(robots[i] + 1 if b==i else robots[i] for i in range(4))
...
# Write a case for mins_remaining = 1 (45x speedup)
...
# 
# if (resources[b]//mins_remaining) + robots[b] >= maxes[b] (30% speedup)
# - if the resources we have in hand plus the robots we have allow us to buld any robot in any remaining min, don't build more of that robot type.
