...
def manhattan3(p, q):
    return int(abs(p[0] - q[0])) + int(abs(p[1] - q[1])) + int(abs(p[2] - q[2]))
...
def test_grid():
    ...
    assert(g == [['0', '1', '2', '2'], ['2', '1', '1', '1'], ['3', '1', '1', '3']])
    print_grid(g, spacing=4)
    print()
    g = grid_ints_from_strs(["0122", "2111", "3113"])
    assert(g == [[0,1,2,2], [2,1,1,1], [3,1,1,3]])
    assert(len([q for q in grid_neighbors((0,0), 3)]) == 2)
    assert(len([q for q in grid_neighbors((2,2), 3)]) == 2)
    assert(len([q for q in grid_neighbors((1,1), 3)]) == 4)
    assert(len([q for q in grid_8_neighbors((0,0), 3)]) == 3)
    assert(len([q for q in grid_8_neighbors((1,1), 3)]) == 8)
    print_grid(g, spacing=3,markfn=lambda r,c,x:"*" if x > 2 else "")
